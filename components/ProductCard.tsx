...
export default function ProductCard({ product }: Props) {
  ...
  const onSale = product.onSale ?? discount > 0;
  ...
  return (
    ...
    >
      {/* Everything except the action buttons is wrapped in a link to the
          product detail page. The buttons live outside the <Link> so we
          never nest an interactive <button> inside an <a>. */}
      ...
      {/* Action row — outside the Link, so these clicks never navigate */}
