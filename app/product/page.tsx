  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (found) {
        ...
        const productReviews = await getReviewsForProduct(found.id);
...
function ReviewsSection({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const ok = await addReview(review);
    setSubmitting(false);
      ...
      {showForm && (
        ...
        >
          ...
          <button
            type="submit"
            disabled={submitting}
            className="bg-navy text-white text-sm font-medium px-5 py-2.5 rounded-full hover:bg-navy-light transition-colors disabled:opacity-50"
          >
